...
// 1. Import OGL from a CDN that supports ES Modules
import { Renderer, Triangle, Program, Mesh } from 'https://esm.sh/ogl';

// 2. Define the settings that would normally be React Props
const settings = {
  height: 3.5,
  baseWidth: 5.5,
  animationType: 'rotate', // options: 'rotate', 'hover', '3drotate'
  glow: 1,
  offset: { x: 0, y: 0 },
  noise: 0.0, // Retaining the user's setting of 0 noise
  transparent: true,
  scale: 3.6,
  hueShift: 0,
  colorFrequency: 1,
...
// 3. Target our background div
const container = document.getElementById('prism-bg');

if (container) {
  initPrism(container, settings);
}

function initPrism(container, opts) {
  const {
    height,
    baseWidth,
    animationType,
